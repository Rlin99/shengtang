...
-- 该语句使用SELECT声明从fruits表中获取名称为f_name和f_price两个字段下的所有水果名称和价格
   select f_name as 水果名称, f_price as 水果价格 from fruits;
-- 查询价格为10.2元的水果的名称
...
-- employee 附加最大工资的列
   select a.*, (select max(b.e_salary) from employee b where a.dept_no = b.dept_no) from employee a;
...
-- 在employee表中，计算不同类型职工的总工资数。
   select dept_no, sum(e_salary) from employee group by dept_no;
...
-- 在employee表中，查询到目前为止，工龄大于等于10年的员工信息。
   select * from employee where hireDate <= '2010-06-23';
...
-- 根据s_id对fruits表中的数据进行分组，将每个供应商的水果名称显示出来
   select s_id, f_name from fruits group by s_id, f_name order by s_id;
...
-- 将以上内容使用ORDER BY关键字按总订单价格排序显示结果
   ...
   from orderitems 
   where (item_price * quantity) > 100 
   order by (item_price * quantity) desc;
-- 显示fruits表查询结果的前4行
...
-- 在orderitems表中查询30005号订单一共购买的水果总量
   select o_num, sum(quantity) from orderitems where o_num = 30005 group by o_num;
...
-- 在fruits表中查找市场上价格最高的水果
   select f_name, f_price from fruits where f_price = (select max(f_price) from fruits group by s_id);
...
select f_name, f_price from fruits where f_price = (select min(f_price) from fruits);
